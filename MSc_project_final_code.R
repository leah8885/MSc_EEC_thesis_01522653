...
outliers_pairs <- subset(pairs_full, log10(Stability)>20)#one numbered outlier and 27 Inf values pruned_small <- subset(small, log10(Stability)<12)# version without outliers 
...
pair_full <- rq(log10(Stability) ~ log10(Distance) * log10(Variance_CD) * Year, data = pruned_pairs, tau = 0.5)
pair_model_2 <- rq(log10(Stability) ~ log10(Distance) * log10(Variance_CD) + Year, data = pruned_pairs, tau =0.5)
anova.rq(pair_full, pair_model_2)#did individual models with quantiles 0.2-0.9

#model of this analysis 
pair_model <- rq(log10(Stability) ~ log10(Distance) * log10(Variance_CD) * Year, data = pruned_pairs, tau = 1:9/10)
...
clust_full <- rq(log10(Stability) ~ log10(Area) * log10(Variance_CD) * Year, data = clusters_final_cleaned_150m, tau = 0.5)
clust_model_2 <- rq(log10(Stability) ~ log10(Area) * log10(Variance_CD) + Year, data = clusters_final_cleaned_150m, tau =0.5)
